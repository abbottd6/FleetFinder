...
VALUES 
    ...
    (SELECT user_name FROM `user` WHERE id_user = (SELECT MIN(id_user) FROM `user`)), (SELECT style_id FROM play_style WHERE play_style = 'Casual'), (SELECT legality_id FROM legality WHERE legality = 'Undefined'), (SELECT group_status_id FROM group_status WHERE group_status = 'Current/Active'),
