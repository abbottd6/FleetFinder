...
VALUES 
	...
	(SELECT style_id FROM play_style WHERE play_style = 'Casual'), (SELECT legality_id FROM legality WHERE legality = 'Undefined'), (SELECT group_status_id FROM group_status WHERE group_status = 'Current/Active'),
...
VALUES 
	...
	(SELECT style_id FROM play_style WHERE play_style = 'Casual'), (SELECT legality_id FROM legality WHERE legality = 'Lawful'), (SELECT group_status_id FROM group_status WHERE group_status = 'Current/Active'),
    ...
    (SELECT planet_id FROM planet_moon_system WHERE planet_name = 'Bloom: Pyro III'), 'Need a turret gunner for my Vanguard Sentinel', '2', 'Gunner', 'REQUIRED', 'Discord');
